...
$ccid = $arr_urinya[4];
if ($sesi['id_peserta'] == '') {
    echo "<script>window.location.href = '" . $inc_ . "/mandiri/detail/" . $ccid . "'</script>";
    exit;
}
    ...
    <div class="container-fluid px-md-4">
        <div class="row g-3">
            ...
            <div class="col-md-12 my-1">
                ...
                ?>
                    <div class="divsoal" id="soal-<?php echo $mid ?>" style="display:none">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <ul class="nav nav-pills row row-cols-5 g-2 text-center" id="pills-tab" role="tablist">
                                                    <?php foreach ($datasoal[$key]['datanya'] as $i => $v) {
                                                    ...
                                                    ?>
                                                        <li class="nav-item col p-1" role="presentation">
                                                            <button class="nav-link text-center mx-auto w-100 <?php echo $active ?> <?php echo 'quest_' . $v['no'] . '_' . $mid ?>" id="pills-<?php echo $i . $mid ?>-tab" data-bs-toggle="pill" data-bs-target="#pills-<?php echo $i . $mid ?>" type="button" role="tab" aria-controls="pills-<?php echo $i . $mid ?>" aria-selected="true"><?php echo ($i + 1) ?></button>
                                                        </li>
                                                    <?php } ?>
                                                </ul>
                                            </div>
...
<script>
    let waktumulai = sessionStorage.getItem('savemulai') ? sessionStorage.getItem('savemulai') : '';
    ...
    let tryoutujian = '';
    tryoutujian = sessionStorage.getItem("tryoutujian");
    if (tryoutujian) {
        jsonsoal = JSON.parse(tryoutujian);
        let no = 0;
    ...
    sessionStorage.setItem("tryoutujian", JSON.stringify(jsonsoal));
    ...
    $(document).ready(function() {
        ...
        if (waktumulai == '') {
            ...
            sessionStorage.setItem('savemulai', "<?php echo $tgl_mulai ?>");
        }
        ...
        let materinya = 0;
        if (sessionStorage.getItem('savemateri')) {
            materinya = atob(sessionStorage.getItem('savemateri'));
        }
    ...
    function mulai(indexnya) {
        ...
        sessionStorage.setItem("savemateri", btoa(indexterpilih));
        ...
        if (sessionStorage.getItem('savetime')) {
            let waktunya = JSON.parse(sessionStorage.getItem('savetime'));
    ...
    function hitung() {
        ...
        sessionStorage.setItem("savetime", '{"jam":"' + jam + '","menit":"' + menit + '","detik":"' + detik + '"}');
        ...
        if (navigator.onLine) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    window.location.href = "<?php echo $inc_ ?>/mandiri/hasil/<?php echo $arr_urinya[4]; ?>";
    ...
    function jawabsoal(kodesoal, jawaban, kodebtn, materi, indexnya) {
        ...
        let tryoutujian = JSON.stringify(jsonsoal);
        sessionStorage.setItem("tryoutujian", tryoutujian);
    }
</script>
