...
if (isset($_POST['btn_pendaftar'])) {
	...
	if ($postcurl_peserta['status'] == '200') {
		echo '<script>swal.fire({icon:"success",title:"' . $postcurl_peserta['message'] . '"})</script>';
	} else if ($postcurl_peserta['status'] == '203') {
		echo '<script>swal.fire({icon:"success",title:"Kamu sudah pernah mendaftar pada event ini"})</script>';
	} else {
			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header title text-center">
						Form Pendaftaran Peserta Ujian Try Out
					</div>
