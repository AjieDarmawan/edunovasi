...
$data['id_peserta'] = $_POST['id_peserta'];
$data['email'] = $_POST['email'];
$data['nama'] = $_POST['nama'];

$dataposisi = array();
$dataposisi['id_event'] = base64_decode($_POST['id_event']);
$dataposisi['email'] = $data['email'];
// eduprint($data);
// exit;
...
$link = '';
$namafile = 'tryout_' . base64_decode($data['id_event']) . '-' . $data['id_peserta'] . '-' . date('Ymdhis') . '.pdf';
$pathpdf = $_SERVER['DOCUMENT_ROOT'] . '/rpt' . '/' . $namafile;
$link = $inc_ . '/rpt' . '/' . $namafile;
// echo $link;
// exit;
...
if (file_exists($pathpdf)) {
    ...
    $skorirt = 'Proses Perhitungan';
    $getcurl_rank = postcurl($base_api_irt . 'check_posisi_rangking', $dataposisi);
    if ($getcurl_rank) {
        $skorirt = $getcurl_rank['skor'];
    }
    ...
    <div style="text-align:left">
        ...
        <table>
            <tr>
                ...
                <td style="width:40%"><b>' . $data['nama'] . '</b></td>
...
if (file_exists($pathpdf)) {
    ...
    $balikan['status'] = '200';
    $balikan['message'] = 'Sukses';
    $balikan['data'] = $link;
    $balikan['namafile'] = $namafile;
    echo json_encode($balikan);
    exit;
} else {
